...
def aggregating_consumer_func(
    current_stream_key, last_message_id, current_hourly_total, current_hourly_count
):
    log(
...
def averages_consumer_func():
    ...
    while True:
        ...
        if response:
            ...
            redis.hset(
                const.AVERAGES_CONSUMER_STATE_KEY,
                mapping={"last_message_id": last_message_id},
            )
        else:
            log(
                AVERAGES_CONSUMER_PREFIX,
...
def main():
    ...
    aggregating_consumer = Process(
        target=aggregating_consumer_func,
        args=(
            current_stream_key,
            last_message_id,
